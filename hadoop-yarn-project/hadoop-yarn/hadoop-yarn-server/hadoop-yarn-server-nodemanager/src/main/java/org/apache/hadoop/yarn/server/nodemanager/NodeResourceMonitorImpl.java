...
public class NodeResourceMonitorImpl extends AbstractService implements
    ...
    private int nodePhyCore;
    private long nodeConfiguredMem;
    private int nodeConfiguredCore;
    ...
    protected void serviceInit(Configuration conf) throws Exception {
        ...
        this.nodeConfiguredMem = conf.getLong(YarnConfiguration.NM_PMEM_MB,
                YarnConfiguration.DEFAULT_NM_PMEM_MB);
        this.nodeConfiguredCore = conf.getInt(YarnConfiguration.NM_VCORES,
                YarnConfiguration.DEFAULT_NM_VCORES);

        // In case we cannot get the real usage, we should use the configured instead
        if (this.nodePhyMem == 0) {
            this.nodePhyMem = this.nodeConfiguredMem;
        }

        if (this.nodePhyCore == 0 ) {
            this.nodePhyCore = this.nodeConfiguredCore;
        }
    ...
    public int getNodeCapacityMemoryMb() {
        return (int) (this.nodePhyMem >> 20); // convert from B to MB
    }

    public int getNodeCapacityRealCores() {
        return this.nodePhyCore;
    }

    public int getNodeConfiguredMemoryMb() {
        return (int) (this.nodeConfiguredMem >> 20); // convert from B to MB
    }

    public int getNodeConfiguredCores() {
        return this.nodeConfiguredCore;
    }
}
