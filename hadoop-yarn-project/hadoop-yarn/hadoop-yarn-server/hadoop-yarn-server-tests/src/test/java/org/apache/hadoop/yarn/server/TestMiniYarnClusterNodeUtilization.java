...
public class TestMiniYarnClusterNodeUtilization {
  ...
  public void testUpdateNodeUtilization()
      ...
      NodeHeartbeatRequest request =
              NodeHeartbeatRequest.newInstance(nodeStatus, null, null);
  ...
  private void verifySimulatedUtilization() throws InterruptedException {
      ...
      for (int i=0; i<100; i++) {
          for (RMNode ni : rmContext.getRMNodes().values()) {
              ResourceUtilization util = ni.getNodeUtilization();
              if (util != null && util.equals(nodeUtilization)) {
                  break;
              }
          }
          Thread.sleep(100);
      ...
      for (RMNode ni : rmContext.getRMNodes().values()) {
          ...
          SchedulerNode schedulerNode =
                  scheduler.getSchedulerNode(ni.getNodeID());
          cu = schedulerNode.getAggregatedContainersUtilization();
          assertEquals("Containers Utillization not propagated to SchedulerNode",
                  containersUtilization, cu);

          nu = schedulerNode.getNodeUtilization();
